		...
		this.each(function(){
			...
			switch ($t.p.treeGridModel) {
				...
				case 'adjacency' :
					...
					$(this.p.data).each(function(){
						//console.log('left: ' + this[dtid]+'\tright: ' + $.jgrid.stripPref($t.p.idPrefix, rc[parent_id]));
						if(this[dtid] === $.jgrid.stripPref($t.p.idPrefix, rc[parent_id]) ) {
							result = this;
							return false;
						}
					});
					//TODO notice ===与==的区别,详情见：http://blog.sina.com.cn/s/blog_4b32835b01014iv9.html
					break;
			}
		});
		return result;
